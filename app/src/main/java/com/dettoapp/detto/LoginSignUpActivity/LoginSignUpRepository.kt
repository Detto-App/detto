    ...
    fun storeUserAndTokenData(context: Context, receivingUserModel: ReceivingUserModel) {
        if (receivingUserModel.teacher != null) {
            storeDataInSharedPreferences(
                context,
                receivingUserModel.teacher.email,
                receivingUserModel.teacher.name,
                Constants.TEACHER,
                receivingUserModel.teacher.uid
            )
            storeDataAsJson(context, receivingUserModel.teacher)
        } else {
            storeDataInSharedPreferences(
                context,
                receivingUserModel.student!!.email,
                receivingUserModel.student.name,
                Constants.STUDENT,
                receivingUserModel.student.uid,
                receivingUserModel.student.susn
            )
